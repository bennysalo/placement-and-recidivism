...
+ 'effect_variable' - A character string. The name of the variable that constitutes the independent variable in later analysis. The matching should thus produce two groups that are balanced on every covariate except this variable.
+ 'balancing_variables' - A vector of character stings. The names of all the variables that serve as covariates. The default is a previously defined vector 'all_predictors'.
...
my_matching <- function(subset_data, effect_variable, balancing_variables = all_predictors, ...) {
 ...
 # Do matching with prespecified parameters
  ...
  # For interpretabilit this should also be centered
  out$data$distance_logit <- log(out$match$distance / (1 - out$match$distance))
  out$data$distance_logit <- out$data$distance_logit - mean(out$data$distance_logit)
...
Let's explore if the effect of the propensity score is linear in the logit. We add a couple of formulas to test. Later let's also write a function to test the different models against each other.

```{r}

add_glm_tests <- function(results_list) {
  # open placement
  if (results_list$effect_variable == "openPrison") {         
    f_ancova      <- "reoffenceThisTerm ~ openPrison + distance_logit"
    f_interaction <- "reoffenceThisTerm ~ openPrison * distance_logit"
    f_non_linear  <- "reoffenceThisTerm ~ openPrison + poly(distance_logit, 2)"
    f_int_non_lin <- "reoffenceThisTerm ~ openPrison * poly(distance_logit, 2)"
  # conditional release  
  } else if (results_list$effect_variable == "conditionalReleaseGranted"){
    ...
    f_non_linear  <- "reoffenceThisTerm ~ conditionalReleaseGranted + poly(distance_logit, 2)"
    f_int_non_lin <- "reoffenceThisTerm ~ conditionalReleaseGranted * poly(distance_logit, 2)"
  ...
  results_list$glm_interaction <- glm(f_interaction, 
                                          family = binomial, data = used_data)
  results_list$glm_non_linear  <- glm(f_non_linear,      
                                          family = binomial, data = used_data)
  results_list$glm_int_non_lin <- glm(f_int_non_lin, 
                                          family = binomial, data = used_data)
...
Function to test all glm models against each other
```{r}
compare_models <- function(results_list) {
    anova(results_list$glm_ancova, 
          results_list$glm_interaction, 
          results_list$glm_int_non_lin, test = "Chisq")
...
Run the glm-models on each results lists (using function defined above `add_glm_test`.
...
Adding both an interaction and a nonlinear effect is an improvement

```{r}
compare_models(op_cr0_ps0)
anova(op_cr0_ps0$glm_ancova, op_cr0_ps0$glm_non_linear, test = "Chisq")
anova(op_cr0_ps0$glm_ancova, op_cr0_ps0$glm_int_non_lin, test = "Chisq")
summary(op_cr0_ps0$glm_ancova)
summary(op_cr0_ps0$glm_int_non_lin)
# summary      (op_cr0_ps0$glm_int_non_lin)
...
```{r}
compare_models(op_cr0_ps1)
anova(op_cr0_ps1$glm_ancova, op_cr0_ps1$glm_non_linear, test = "Chisq")
anova(op_cr0_ps1$glm_ancova, op_cr0_ps1$glm_int_non_lin, test = "Chisq")
...
No main effect. Adding interaction term is not a statistically significant improvement.
Nonlinear-effect is statistically significant.

```{r}
compare_models(cr_op1_ps0)
anova(cr_op1_ps0$glm_ancova, cr_op1_ps0$glm_non_linear, test = "Chisq")
anova(cr_op1_ps0$glm_ancova, cr_op1_ps0$glm_int_non_lin, test = "Chisq")
anova(cr_op1_ps0$glm_non_linear, cr_op1_ps0$glm_int_non_lin, test = "Chisq")
# summary      (cr_op1_ps0$glm_ancova)
# get_chi_stats(cr_op1_ps0$glm_ancova)
# 
summary      (cr_op1_ps0$glm_non_linear)
...
```{r}
compare_models(cr_op1_ps1)
anova(cr_op1_ps1$glm_ancova, cr_op1_ps1$glm_non_linear, test = "Chisq")
anova(cr_op1_ps1$glm_ancova, cr_op1_ps1$glm_int_non_lin, test = "Chisq")
anova(cr_op1_ps1$glm_non_linear, cr_op1_ps1$glm_int_non_lin, test = "Chisq")
...
plot_data <- data.frame(
  ...
  est_prob_recidivism = predict(results_list$glm_int_non_lin, 
                                type = "response",
                                newdata = results_list$data),
